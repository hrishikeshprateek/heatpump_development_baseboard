...
void loadSystemState() {
    ...
    if (currentState.targetTime > now.unixtime()) {
        ...
        // Reset previous control variables to avoid inheriting stale states
        prevHeater = false;
        prevFanSpeed = 0;
...
void fuzzyControl() {
    ...
    // If RH is significantly above target, force higher fan speed; if too low, lower it and optionally engage humidifiers.
...
void completeRecipe() {
    ...
    // Reset the control state and recipe flag
    recipeReceived = false;
    prevHeater = false;
    prevFanSpeed = 0;
    
    // Clear currentState and save to EEPROM
    memset(&currentState, 0, sizeof(SystemState));
    saveSystemState();
...
void sendStatus(SoftwareSerial &serialPort) {
    ...
    float currentTemp = dht.readTemperature();
    float currentHumidity = dht.readHumidity();
    long timeRemaining = currentState.targetTime - now.unixtime(); 
